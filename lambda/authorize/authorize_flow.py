...
def handler(event, context):
    ...
    config["idp_auth_params"] = os.environ.get("IdpAuthParams")
    config["idp_response_uri"] = os.environ.get("CognitoIdpResponseUri")
    ...
    # Redirect URI parameter is set to the Cognito IdP response URI by default. If PKCE is enabled, it will be set to Proxy Callback URI.
    params["redirect_uri"] = config["idp_response_uri"]
    
    # Add additional OIDC IdP custom request parameters for authorization
    params["nonce"] = params["state"]
    idp_auth_params_dict = json.loads(config["idp_auth_params"])
    for key, value in idp_auth_params_dict.items():
       params.update({key: value})

    # Generate code_verifier and code_challenge for PKCE flow and store state in dynamodb table with ttl
    if pkce_toggle:
